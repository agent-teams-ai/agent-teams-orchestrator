...
async function makeSourceFixture({ installPackages = true } = {}) {
  ...
  await Promise.all([
    ...
    writeFile(path.join(root, "package.json"), `${JSON.stringify({ name: "docs-parity-fixture", private: true, type: "module", scripts: Object.fromEntries(["check", "doctor", "find", "info", "new", "recover"].map((command) => [`docs:${command}`, `agent-teams-docs ${command} --consumer . --profile architecture/foundation/docs-protocol.yaml`])), devDependencies: { "@agent-teams/docs-protocol": "0.1.0-rc.10", "@agent-teams/engineering-foundation": "0.17.0-rc.7" } }, null, 2)}\n`),
