...
async function makeSourceFixture({ installPackages = true } = {}) {
  ...
  await Promise.all([
    ...
    writeFile(path.join(root, "package.json"), `${JSON.stringify({ name: "docs-parity-fixture", private: true, type: "module", scripts: Object.fromEntries(["check", "doctor", "find", "info", "new", "recover"].map((command) => [`docs:${command}`, `agent-teams-docs ${command} --consumer . --profile architecture/foundation/docs-protocol.yaml`])), devDependencies: { "@agent-teams/docs-protocol": "0.1.1", "@agent-teams/engineering-foundation": "0.17.0" } }, null, 2)}\n`),
