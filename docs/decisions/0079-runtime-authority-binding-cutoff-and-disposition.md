...
AR has not yet accepted a pre-materialization negative operation-intent guard
for the case where prevention reaches AR before the original operation command.
This ADR requires that capability, or an AR-owned equivalent with the same
linearization and resurrection-prevention guarantees, before its early-cutoff
path can be accepted. Orchestrator core must consume it through a Run-owned port
and cannot duplicate the guard or its retention state.
...
If accepted, this ADR supersedes ADR-0008. Until then ADR-0008 remains the
accepted decision and this document is review material only.
...
| Stable Orchestrator project identity, project-level runtime placement, binding generation, scope admission intent, and runtime-scope disposition intent | Proposed Orchestration Scope bounded context | Orchestration scope state and runtime owner-receipt references |
...
The proposed Orchestration Scope context is a supporting domain. It does not own
...
The required AR contract must durably record a negative operation-intent guard,
keyed by the scoped original command identity and target-intent correlation.
Dispatch claim and this guard must serialize in the same AR authority boundary.
A scoped `not found` without its durable receipt is not completion evidence; any
later matching dispatch must be rejected before provider side effect. Until AR
accepts this semantic contract and publishes producer fixtures, this path remains
an explicit ADR-0079 acceptance blocker rather than an assumed capability.
...
creates the durable prevent-dispatch fence described above. A response that only
states that no RuntimeOperation currently exists cannot satisfy the predecessor
barrier.
...
- cutoff arriving before the original dispatch and leaving a durable negative
  dispatch fence that rejects the delayed command before provider side effect;
...
This proposed ADR cannot become accepted in isolation. The accepting change set
must also:

- mark ADR-0008 as superseded and preserve only its compatible port and ACL
  principles through this decision;
- preserve ADR-0080's accepted Orchestration Scope identity, admission, binding,
  and whole-Project disposition ownership;
- require an accepted AR producer contract and fixtures for cutoff that wins
  before operation materialization, or remove that early-cutoff claim and prove
  an alternative with equivalent no-late-dispatch guarantees;
- synchronize the runtime boundary, reliability ownership, package catalog, and
  affected open decisions without materializing packages whose use cases have not
  started;
- keep product and jurisdiction policy sources, legal-hold semantics, retention
